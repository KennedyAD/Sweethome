 ...
 * @param callback The function that will called once the 3D model is loaded.
 */
      ...
      if (objEntryName == null) {
        ...
        for (var i in entries) {
          if (entries [i].name.match(/\.obj$/)) {
            loader.parseOBJEntry(entries [i], zip, callback);
            return;
          } 
        }
      } else {
        loader.parseOBJEntry(zip.file(objEntryName), zip, callback);
      }
    } catch (err) {
      callback(err, null);
    }
...
OBJLoader.prototype.parseOBJEntry = function(objEntry, zip, callback) {
  ...
  var appearances = {};
  for (var k in defaultAppearances) {
    var appearance = defaultAppearances [k];
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    var start = strings [0];
    if (start === "v") {
      vertices.push(parseVector3f(strings));
    } else if (start === "vt") {
      textureCoordinates.push(parseVector2f(strings));
    } else if (start === "vn") {
      normals.push(parseVector3f(strings));
    } else if (start === "l") {
  ...
  var scene = this.createScene(vertices, textureCoordinates, normals, groups, appearances);
  callback(null, scene);
}
...
OBJLoader.prototype.parseMaterialEntry = function(mtlEntryName, appearances, objEntry, zip) {
  ...
  parseMaterial(mtlEntry.asBinary(), appearances, objEntry, zip)
}
  ...
  for (var key in groups) {
    ...
    if (geometries.length > 0) {
      ...
      while (i < geometries.length) {
        ...
        var appearance = getAppearance(appearances, firstGeometryMaterial);
        ...
        while (++max < geometries.length) {
          ...
          if ((geometry.constructor !== firstGeometry.constructor)
              ...
              || material !== null && getAppearance(appearances, material) !== appearance
        ...
        if (firstGeometry instanceof OBJFace) {
          ...
          var geometryArray;
          if (onlyTriangles) {
            geometryArray = new IndexedTriangleArray3D(vertices, textureCoordinates, normals,   
                coordinatesIndices, textureCoordinateIndices, normalIndices);
          } else {
          ...
          for (var j = 0, index = 0; j < geometryCount; index += stripCounts [j], j++) {
            ...
            shape.addGeometry(new IndexedLineArray3D(vertices, textureCoordinates,  
                lineCoordinatesIndices, lineTextureCoordinateIndices));
          }
        }
...
function parseVector3f(strings) {
  //     v x y z
  // or vn x y z
  // or Ka r g b
  // or Kd r g b
...
function parseVector2f(strings) {
  // vt x y z
  return vec2.fromValues(parseFloat(strings [1]), parseFloat(strings [2]));
}

function parseMaterial(mtlContent, appearances, objEntry, zip) {
  ...
  for (var i in lines) {
    ...
    if (start === "newmtl") {
      ...
      if (start === "Ka") {
        currentAppearance.ambiantColor = parseVector3f(strings);
      } else if (start === "Kd") {
        currentAppearance.diffuseColor = parseVector3f(strings);
      } else if (start === "Ks") {
        currentAppearance.specularColor = parseVector3f(strings);
      } else if (start === "Ns") {
        ...
        currentAppearance.texture = new Image();
        if (imageEntryName.toLowerCase().indexOf(".png") === imageEntryName.length - 4) {
          currentAppearance.texture.src = "data:image/png;base64," + base64Image;
        } else {
          currentAppearance.texture.src = "data:image/jpeg;base64," + base64Image;
        }
      } 
...
function getAppearance(appearances, material) {
  var appearance = undefined;
  if (material !== null) {
    appearance = appearances [material];
  }
  if (appearance === undefined) {
    appearance = defaultAppearances ["default"];
  }
  return appearance;
}
 ...
 * @constructor
 */
function IndexedGeometryArray3D(vertices, textureCoordinates,  
                                vertexIndices, textureCoordinateIndices) {
 ...
 * @constructor
 */
function IndexedLineArray3D(vertices, textureCoordinates,  
                            vertexIndices, textureCoordinateIndices) {
  IndexedGeometryArray3D.call(this, vertices, textureCoordinates, vertexIndices, textureCoordinateIndices);
}
 ...
 * @constructor
 */
function IndexedTriangleArray3D(vertices, textureCoordinates, normals, 
                                vertexIndices, textureCoordinateIndices, normalIndices) {
  IndexedGeometryArray3D.call(this, vertices, textureCoordinates, vertexIndices, textureCoordinateIndices);
...
Node3D.prototype.getNormalizedTransformGroup = function(modelRotation, width) {
  return new TransformGroup3D(this.getNormalizedTransform(modelRotation, width));
}

Node3D.prototype.getNormalizedTransform = function(modelRotation, width) {
  // Get model bounding box size 
  var modelBounds = this.getBounds();
  ...
  if (modelRotation !== null) {
    ...
    var rotatedModelBounds = this.getTransformedBounds(modelTransform);
...
Node3D.prototype.getMinimumSize = function() {
  return 0.001;
}

Node3D.prototype.getRotationTransformation = function(modelRotation) {
...
Node3D.prototype.getBounds = function() {
  return this.getTransformedBounds(mat4.create());
}

Node3D.prototype.getTransformedBounds = function(transformation) {
  var objectBounds = new BoundingBox3D(
      vec3.fromValues(Infinity, Infinity, Infinity), 
      vec3.fromValues(-Infinity, -Infinity, -Infinity));
  this.computeBounds(objectBounds, transformation, !this.isOrthogonalRotation(transformation));
  return objectBounds;
}

Node3D.prototype.isOrthogonalRotation = function(transformation) {
  for (var i = 0; i < 3; i++) {
    for (var j = 0; j < 3; j++) {
 ...
 * @constructor
 */
function Shape3D(name, geometry3D, appearance3D) {
  this.name = name;
  this.geometries = geometry3D !== null ? [geometry3D] : [];
  this.appearance = appearance3D;
  this.bounds = null;
}
...
Shape3D.prototype.getBounds = function() {
  if (this.bounds === null) {
    this.bounds = this.computeTransformedGeometryBounds(null);
  }
  return this.bounds.clone();
}

Shape3D.prototype.computeBounds = function(bounds, parentTransformations, transformShapeGeometry) {
  var shapeBounds;
  if (transformShapeGeometry) {
    shapeBounds = this.computeTransformedGeometryBounds(parentTransformations);
  } else {
...
Shape3D.prototype.computeTransformedGeometryBounds = function(transformation) {
...
Shape3D.prototype.updateBounds = function(vertex, transformation, lower, upper) {
...
Group3D.prototype.computeBounds = function(bounds, parentTransformations, transformShapeGeometry) {
  // Compute the bounds of all the node children
  for (var i in this.children) {
    this.children [i].computeBounds(bounds, parentTransformations, transformShapeGeometry);
  }
}
...
TransformGroup3D.prototype.computeBounds = function(bounds, parentTransformations, transformShapeGeometry) {
  parentTransformations = mat4.clone(parentTransformations);
  mat4.mul(parentTransformations, parentTransformations, this.transform);
  // Compute the bounds of all the node children
  for (var i in this.children) {
    this.children [i].computeBounds(bounds, parentTransformations, transformShapeGeometry);
  }
