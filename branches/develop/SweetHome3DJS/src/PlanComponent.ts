...
class PlanComponent implements PlanView {
    ...
    static HIDPI_SCALE_FACTOR : number = 2;
    ...
    addMouseListeners(controller : PlanController) {
      ...
      var mouseListener = {
        ...
        mouseWheelMoved : function(ev : MouseWheelEvent) {
          ...
          let shortcutKeyPressed: boolean = OperatingSystem.isMacOSX() ? ev.metaKey : ev.ctrlKey;
          if(shortcutKeyPressed) {
            let mouseX = planComponent.convertXPixelToModel((<any>ev).canvasX);
            let mouseY = planComponent.convertYPixelToModel((<any>ev).canvasY);
            
            let oldScale : number = planComponent.getScale();
            controller.zoom((<any>ev).wheelRotation < 0?Math.pow(1.05, -(<any>ev).wheelRotation):Math.pow(0.95, (<any>ev).wheelRotation));
            if(planComponent.getScale() !== oldScale) {
              ...
              let mouseDeltaX = (<any>ev).canvasX - planComponent.convertXModelToPixel(mouseX);
              let mouseDeltaY = (<any>ev).canvasY - planComponent.convertYModelToPixel(mouseY);
...
//  }
    ...
    installDefaultKeyboardActions() {
      ...
      this.container.addEventListener("keydown", ev => this.callAction(ev, "keydown"), true);
      this.container.addEventListener("keyup", ev => this.callAction(ev, "keyup"), true);
    }
     ...
     * @private 
     */
    private callAction(ev: any, keyType: string) {
     ...
     * @private
     */
    createActions(controller : PlanController) {
      let planComponent = this;
      
      function MoveSelectionAction(dx : number, dy : number) {
        this.dx = dx;
        this.dy = dy;
      }
      ...
      function ToggleMagnetismAction(toggle: boolean) {
        this.toggle = toggle;
      }
      ...
      function SetAlignmentActivatedAction(alignmentActivated: boolean) {
      ...
      function SetDuplicationActivatedAction(duplicationActivated: boolean) {
      ...
      function SetEditionActivatedAction(editionActivated: boolean) {
