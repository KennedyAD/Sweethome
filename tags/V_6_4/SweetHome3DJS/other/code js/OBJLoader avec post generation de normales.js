  ...
  JSZipUtils.getBinaryContent(url, function(err, data) {
    ...
    try {
      if (objEntryName == null) {
        ...
        loader.parseOBJEntry(zip.file(objEntryName), zip, url, modelObserver);
      }
    } catch (err) {
        modelObserver.modelError(err);
    }
...
OBJLoader.prototype.createScene = function(vertices, textureCoordinates, normals, groups, appearances, onprogression) {
  ...
  for (var key in groups) {
    ...
    if (Math.floor((builtGroupCount - 1) / groupCount * 10) < Math.floor(builtGroupCount / groupCount * 10)) {
      onprogression(BUILDING_MODEL, "", builtGroupCount / groupCount);
    }
      ...
      while (i < geometries.length) {
        ...
        if (firstGeometry instanceof OBJFace) {
          ...
          if (onlyTriangles) {
                  ...
                  for ( ; k < polygonVertexIndices.length; k++) {
                    ...
                    if (vector0 [0] === 0 && vector0 [1] === 0 && vector0 [2] === 0) {
                      ...
                      if (vector1 [0] === 0 && vector1 [1] === 0 && vector1 [2] === 0) {
                        ...
                        if (dot01 > 0
                            /* TODO check angle is not reflex 
            ...
            if (!firstFaceHasNormalIndices) {
              stripCounts = [];
              for (var j = 0; j < triangleCoordinateIndices.length / 3; j++) {
                stripCounts [j] = 3;
              }
              this.generateNormals(vertices, triangleCoordinateIndices, normals, triangleNormalIndices, stripCounts, group.smooth);
            }
...
OBJLoader.prototype.parseOBJEntry = function(objEntry, zip, zipUrl, modelObserver) {
  ...
  if (typeof(Worker) !== "undefined") { 
    ...
    baseUrl = baseUrl.substring(0, baseUrl.lastIndexOf('/') + 1) + "lib/"; 
    var blob = new Blob([ "importScripts('" + baseUrl + "Node3D.js', \n"
  ...
  if (node instanceof Group3D) {
    ...
    if (imageEntryName !== undefined) {
      ...
      if (imageEntryName in images) {
        node.appearance.textureImage = images [imageEntryName];
      } else { 
        var image = new Image();
        node.appearance.textureImage = image;
