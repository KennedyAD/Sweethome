 ...
 * @param {string} url The URL of a zip file containing a 3DS entry that will be loaded
 *            or an URL noted as jar:url!/3dsEntry where 3dsEntry will be loaded.
 ...
 */
Max3DSLoader.prototype.load = function(url, synchronous, modelObserver) {
  if (modelObserver === undefined) {
    modelObserver = synchronous;
    synchronous = false;
  }
  var max3dsEntryName = null;
  if (url.indexOf("jar:") === 0) {
    var entrySeparatorIndex = url.indexOf("!/");
    max3dsEntryName = url.substring(entrySeparatorIndex + 2);
  ...
  var zipObserver = {
      zipReady: function(zip) {
        try {
          if (max3dsEntryName === null) {
            // Search a 3DS entry
            var entries = zip.file(/.*/);
            for (var i = 0; i < entries.length; i++) {
              if (entries [i].name.toLowerCase().match(new RegExp("\." + "3ds" + "$"))) {
                loader.parse3DSEntry(entries [i], zip, url, synchronous, modelObserver);
                return;
              } 
            }
          } else {
            loader.parse3DSEntry(zip.file(decodeURIComponent(max3dsEntryName)), zip, url, synchronous, modelObserver);
          }
        } catch (ex) {
 ...
 */
Max3DSLoader.prototype.clear = function() {
  this.waitingParsed3DSEntries = [];
}
 ...
 * @private
 */
Max3DSLoader.prototype.parse3DSEntry = function(max3dsEntry, zip, zipUrl, synchronous, modelObserver) {
  if (synchronous) { 
    var max3dsContent = max3dsEntry.asUint8Array();
    modelObserver.progression(Node3D.READING_MODEL, max3dsEntry.name, 1);
    var scene = this.parse3DSEntryScene(max3dsContent, max3dsEntry.name, zip, null, modelObserver.progression);
    this.loadTextureImages(scene, {}, zip, zipUrl, synchronous);
    modelObserver.modelLoaded(scene);
  } else {
    var parsed3DSEntry = {"max3dsEntry": max3dsEntry, 
                          "zip": zip, 
                          "zipUrl": zipUrl, 
                          "modelObserver": modelObserver};
    this.waitingParsed3DSEntries.push(parsed3DSEntry);
 ...
 * @private
 */
Max3DSLoader.prototype.parseNextWaitingEntry = function() {
  if (!this.parserBusy) {
    // Parse 3DS files one at a time to avoid keeping in memory unzipped content not yet used
    for (var key in this.waitingParsed3DSEntries) {
      if (this.waitingParsed3DSEntries.hasOwnProperty(key)) {
        var parsed3DSEntry = this.waitingParsed3DSEntries [key];
        var max3dsEntryName = parsed3DSEntry.max3dsEntry.name;
        var zip = parsed3DSEntry.zip;
        // Get 3DS content to parse
        var max3dsContent = parsed3DSEntry.max3dsEntry.asUint8Array();
        parsed3DSEntry.modelObserver.progression(Node3D.READING_MODEL, max3dsEntryName, 1);
        var loader = this;
        // Post future work (avoid worker because the amount of data to transfer back and forth slows the program) 
        setTimeout(
            function() {
              loader.parse3DSEntryScene(max3dsContent, max3dsEntryName, zip, 
                  function(scene) {
                      loader.loadTextureImages(scene, {}, parsed3DSEntry.zip, parsed3DSEntry.zipUrl);
                      parsed3DSEntry.modelObserver.modelLoaded(scene);
                      loader.parserBusy = false;
                      loader.parseNextWaitingEntry();
                    },
                  parsed3DSEntry.modelObserver.progression);
            }, 0);
        
        this.parserBusy = true;
        // Remove parsed entry from waiting list
        delete this.waitingParsed3DSEntries [key];
        break;
      }
    }
  }
 ...
 * @private
 */
Max3DSLoader.prototype.loadTextureImages = function(node, images, zip, zipUrl, synchronous) {
 ...
 * @private
 */
Max3DSLoader.prototype.parse3DSEntryScene = function(max3dsContent, max3dsEntryName, zip, onmodelloaded, onprogression) {
  var meshes = [];       // Mesh3DS 
