...
OBJLoader.prototype.createScene = function(vertices, textureCoordinates, normals, groups, appearances, onmodelcreated, onprogression) {
  ...
  if (onmodelcreated === null) {
    ...
    var sceneBuilder = function() {
        onprogression(Node3D.BUILDING_MODEL, "", builtGeometryCount / groupsGeometryCount);
        var start = Date.now();
        for (var key in groups) {
 ...
 * @private
 */
OBJLoader.prototype.generateNormals = function(vertices, coordinatesIndices, normals, normalIndices, creaseAngle) {
...
OBJLoader.prototype.loadTextureImages = function(node, images, zip, zipUrl, synchronous) {
  if (node instanceof Group3D) {
    for (var i = 0; i < node.children.length; i++) {
      this.loadTextureImages(node.children [i], images, zip, zipUrl);
    }
