 ...
 * @param {java.io.File} furniturePluginFolder
 * @class
 * @extends FurnitureCatalog
 * @author Emmanuel Puybaret
 */
 var DefaultFurnitureCatalog = /** @class */ (function (_super) {
  __extends(DefaultFurnitureCatalog, _super);

  function DefaultFurnitureCatalog(preferences, furniturePluginUrls) {
    _super.call(this);
    var furnitureHomonymsCounter = {};
    var identifiedFurniture = [];
    this.readDefaultFurnitureCatalogs(preferences, furnitureHomonymsCounter, identifiedFurniture);
    if (furniturePluginUrls != null) {
      for (var i = 0; i < furniturePluginUrls.length; i++) {
        this.readPluginFurnitureCatalog(furniturePluginUrls[i], identifiedFurniture);
      }
    }
    return this;
  }
   ...
   * @param {string} pluginFurnitureCatalogURL
   * @param {Object[]} identifiedFurniture
   * @private
   */
   DefaultFurnitureCatalog.prototype.readPluginFurnitureCatalog = function (pluginFurnitureCatalogURL, identifiedFurniture) {
      // TODO
      // try {
      //     var pluginFurnitureCatalogUrl = void 0;
      //     var urlModificationDate = pluginFurnitureCatalogFile.lastModified();
      //     var urlUpdate = DefaultFurnitureCatalog.pluginFurnitureCatalogUrlUpdates.get(pluginFurnitureCatalogFile);
      //     if (pluginFurnitureCatalogFile.canWrite() && (urlUpdate == null || urlUpdate.openConnection().getLastModified() < urlModificationDate)) {
      //         var contentCopy = com.eteks.sweethome3d.tools.TemporaryURLContent.copyToTemporaryURLContent(new com.eteks.sweethome3d.tools.URLContent(pluginFurnitureCatalogFile.toURI().toURL()));
      //         var temporaryFurnitureCatalogUrl = contentCopy.getURL();
      //         DefaultFurnitureCatalog.pluginFurnitureCatalogUrlUpdates.put(pluginFurnitureCatalogFile, temporaryFurnitureCatalogUrl);
      ...
      //     }
      //     var urlLoader = new URLContentClassLoader(pluginFurnitureCatalogUrl);
      //     var resourceBundle = java.util.ResourceBundle.getBundle(DefaultFurnitureCatalog.PLUGIN_FURNITURE_CATALOG_FAMILY, java.util.Locale.getDefault(), urlLoader);
      //     this.libraries.add(0, new DefaultLibrary(pluginFurnitureCatalogFile.getCanonicalPath(), UserPreferences.FURNITURE_LIBRARY_TYPE, resourceBundle));
      //     this.readFurniture(resourceBundle, pluginFurnitureCatalogUrl, null, identifiedFurniture);
      // }
      // catch (__e) {
      //     if (__e != null && __e instanceof java.util.MissingResourceException) {
      //         var ex = __e;
      //     }
      //     if (__e != null && __e instanceof IllegalArgumentException) {
      //         var ex = __e;
      //     }
   ...
   * @param {UserPreferences} preferences
   * @param {Object} furnitureHomonymsCounter
   * @param {Object} identifiedFurniture
   * @private
   */
   DefaultFurnitureCatalog.prototype.readDefaultFurnitureCatalogs = function (preferences, furnitureHomonymsCounter, identifiedFurniture) {
    this.readFurnitureCatalog("DefaultFurnitureCatalog", preferences, furnitureHomonymsCounter, identifiedFurniture);
      // var defaultFurnitureCatalogFamily = (function (c) { return c["__class"] ? c["__class"] : c["name"]; })(DefaultFurnitureCatalog);
      // var classPackage = defaultFurnitureCatalogFamily.substring(0, defaultFurnitureCatalogFamily.lastIndexOf("."));
      // this.readFurnitureCatalog(classPackage + "." + DefaultFurnitureCatalog.CONTRIBUTED_FURNITURE_CATALOG_FAMILY, preferences, furnitureHomonymsCounter, identifiedFurniture);
      // this.readFurnitureCatalog(classPackage + "." + DefaultFurnitureCatalog.ADDITIONAL_FURNITURE_CATALOG_FAMILY, preferences, furnitureHomonymsCounter, identifiedFurniture);
    };
   ...
   * @private
   */
   DefaultFurnitureCatalog.prototype.readFurnitureCatalog = function (furnitureCatalogFamily, preferences, furnitureHomonymsCounter, identifiedFurniture) {
   ...
   */
   DefaultFurnitureCatalog.prototype.isDefaultProperty = function (keyPrefix) {
    if(DefaultFurnitureCatalog.PropertyKey [keyPrefix]) {
      return true;
    } else {
   ...
   DefaultFurnitureCatalog.prototype.readPieceOfFurniture = function (resource, index, furnitureCatalogUrl, furnitureResourcesUrlBase) {
    ...
    if (creationDateString != null) {
      try {
          ...
          var icon = this.getContent(resource, this.getKey(DefaultFurnitureCatalog.PropertyKey.ICON, index), this.getKey(DefaultFurnitureCatalog.PropertyKey.ICON_DIGEST, index), furnitureCatalogUrl, furnitureResourcesUrlBase, false, false);
          var planIcon = this.getContent(resource, this.getKey(DefaultFurnitureCatalog.PropertyKey.PLAN_ICON, index), this.getKey(DefaultFurnitureCatalog.PropertyKey.PLAN_ICON_DIGEST, index), furnitureCatalogUrl, furnitureResourcesUrlBase, false, true);
          ...
          var model = this.getContent(resource, this.getKey(DefaultFurnitureCatalog.PropertyKey.MODEL, index), this.getKey(DefaultFurnitureCatalog.PropertyKey.MODEL_DIGEST, index), furnitureCatalogUrl, furnitureResourcesUrlBase, multiPartModel, false);
          ...
          if (doorOrWindow) {
            var doorOrWindowCutOutShape = this.getOptionalString(resource, this.getKey(DefaultFurnitureCatalog.PropertyKey.DOOR_OR_WINDOW_CUT_OUT_SHAPE, index), null);
            var wallThicknessPercentage = this.getOptionalFloat(resource, this.getKey(DefaultFurnitureCatalog.PropertyKey.DOOR_OR_WINDOW_WALL_THICKNESS, index), depth) / depth;
            var wallDistancePercentage = this.getOptionalFloat(resource, this.getKey(DefaultFurnitureCatalog.PropertyKey.DOOR_OR_WINDOW_WALL_DISTANCE, index), 0) / depth;
            var wallCutOutOnBothSides = this.getOptionalBoolean(resource, this.getKey(DefaultFurnitureCatalog.PropertyKey.DOOR_OR_WINDOW_WALL_CUT_OUT_ON_BOTH_SIDES, index), true);
            var widthDepthDeformable = this.getOptionalBoolean(resource, this.getKey(DefaultFurnitureCatalog.PropertyKey.DOOR_OR_WINDOW_WIDTH_DEPTH_DEFORMABLE, index), true);
            var sashes = this.getDoorOrWindowSashes(resource, index, width, depth);
            return new CatalogDoorOrWindow(id, name, description, information, tags, creationDate, grade, icon, planIcon, model, width, depth, height, elevation, dropOnTopElevation, movable, doorOrWindowCutOutShape, wallThicknessPercentage, wallDistancePercentage, wallCutOutOnBothSides, widthDepthDeformable, sashes, modelRotation, false, modelSize, creator, resizable, deformable, texturable, price, valueAddedTaxPercentage, currency, additionalProperties);
          }
          else {
            var lightSources = this.getLightSources(resource, index, width, depth, height);
            if (lightSources != null) {
              return new CatalogLight(id, name, description, information, tags, creationDate, grade, icon, planIcon, model, width, depth, height, elevation, dropOnTopElevation, movable, lightSources, staircaseCutOutShape, modelRotation, false, modelSize, creator, resizable, deformable, texturable, horizontallyRotatable, price, valueAddedTaxPercentage, currency, additionalProperties);
            }
            else {
              return new CatalogPieceOfFurniture(id, name, description, information, tags, creationDate, grade, icon, planIcon, model, width, depth, height, elevation, dropOnTopElevation, movable, staircaseCutOutShape, modelRotation, false, modelSize, creator, resizable, deformable, texturable, horizontallyRotatable, price, valueAddedTaxPercentage, currency, additionalProperties);
            }
          }
        };
   ...
   */
   DefaultFurnitureCatalog.prototype.getContent = function (resource, contentKey, contentDigestKey, furnitureUrl, resourceUrlBase, multiPartModel, optional) {
    var contentFile = optional ? this.getOptionalString(resource, contentKey, null) : CoreTools.getStringFromKey(resource, contentKey);
    ...
    try {
      ...
      if (resourceUrlBase == null) {
        ...
        if (contentFile.indexOf('!') >= 0 && !(function (str, searchString, position) {
      ...
      if (furnitureUrl == null) {
        content = new URLContent(DefaultFurnitureCatalog, contentFile, multiPartModel);
      }
      else {
        try {
          content = new URLContent(new String("jar:" + furnitureUrl + "!" + contentFile), multiPartModel);
        } catch (ex2) {
   ...
   */
   DefaultFurnitureCatalog.prototype.getModelRotation = function (resource, key) {
    try {
      ...
      if (values.length === 9) {
        return [[parseFloat(values[0]), parseFloat(values[1]), parseFloat(values[2])], [parseFloat(values[3]), parseFloat(values[4]), parseFloat(values[5])], [parseFloat(values[6]), parseFloat(values[7]), parseFloat(values[8])]];
      } else {
        return null;
      }
    } catch (e) {
   ...
   DefaultFurnitureCatalog.prototype.getDoorOrWindowSashes = function (resource, index, doorOrWindowWidth, doorOrWindowDepth) {
    ...
    if (sashXAxisString != null) {
      ...
      if (sashYAxisValues.length !== sashXAxisValues.length) {
        throw new IllegalArgumentException("Expected " + sashXAxisValues.length + " values in " + this.getKey(DefaultFurnitureCatalog.PropertyKey.DOOR_OR_WINDOW_SASH_Y_AXIS, index) + " key");
      }
      ...
      if (sashWidths.length !== sashXAxisValues.length) {
        throw new IllegalArgumentException("Expected " + sashXAxisValues.length + " values in " + this.getKey(DefaultFurnitureCatalog.PropertyKey.DOOR_OR_WINDOW_SASH_WIDTH, index) + " key");
      }
      ...
      if (sashStartAngles.length !== sashXAxisValues.length) {
        throw new IllegalArgumentException("Expected " + sashXAxisValues.length + " values in " + this.getKey(DefaultFurnitureCatalog.PropertyKey.DOOR_OR_WINDOW_SASH_START_ANGLE, index) + " key");
      }
      ...
      if (sashEndAngles.length !== sashXAxisValues.length) {
        throw new IllegalArgumentException("Expected " + sashXAxisValues.length + " values in " + this.getKey(DefaultFurnitureCatalog.PropertyKey.DOOR_OR_WINDOW_SASH_END_ANGLE, index) + " key");
      }
      sashes = (function (s) { var a = []; while (s-- > 0)
        a.push(null); return a; })(sashXAxisValues.length);
        for (var i = 0; i < sashes.length; i++) {
          sashes[i] = new Sash(parseFloat(sashXAxisValues[i]) / doorOrWindowWidth, parseFloat(sashYAxisValues[i]) / doorOrWindowDepth, parseFloat(sashWidths[i]) / doorOrWindowWidth, (function (x) { return x * Math.PI / 180; })(parseFloat(sashStartAngles[i])), (function (x) { return x * Math.PI / 180; })(parseFloat(sashEndAngles[i])));
        }
      }
      else {
        sashes = [];
   ...
   DefaultFurnitureCatalog.prototype.getLightSources = function (resource, index, lightWidth, lightDepth, lightHeight) {
    ...
    if (lightSourceXString != null) {
      ...
      if (lightSourceY.length !== lightSourceX.length) {
        throw new IllegalArgumentException("Expected " + lightSourceX.length + " values in " + this.getKey(DefaultFurnitureCatalog.PropertyKey.LIGHT_SOURCE_Y, index) + " key");
      }
      ...
      if (lightSourceZ.length !== lightSourceX.length) {
        throw new IllegalArgumentException("Expected " + lightSourceX.length + " values in " + this.getKey(DefaultFurnitureCatalog.PropertyKey.LIGHT_SOURCE_Z, index) + " key");
      }
      ...
      if (lightSourceColors.length !== lightSourceX.length) {
        throw new IllegalArgumentException("Expected " + lightSourceX.length + " values in " + this.getKey(DefaultFurnitureCatalog.PropertyKey.LIGHT_SOURCE_COLOR, index) + " key");
      }
      ...
      if (lightSourceDiametersString != null) {
        ...
        if (lightSourceDiameters.length !== lightSourceX.length) {
          throw new IllegalArgumentException("Expected " + lightSourceX.length + " values in " + this.getKey(DefaultFurnitureCatalog.PropertyKey.LIGHT_SOURCE_DIAMETER, index) + " key");
        }
      }
      else {
        lightSourceDiameters = null;
      }
      lightSources = (function (s) { var a = []; while (s-- > 0)
        a.push(null); return a; })(lightSourceX.length);
        for (var i = 0; i < lightSources.length; i++) {
          var color = (function (str, searchString, position) {
            if (position === void 0) { position = 0; }
            return str.substr(position, searchString.length) === searchString;
          })(lightSourceColors[i], "#") ? parseInt(lightSourceColors[i].substring(1), 16) : parseInt(lightSourceColors[i]);
          lightSources[i] = new LightSource(
            ...
            color, 
            lightSourceDiameters != null ? parseFloat(lightSourceDiameters[i]) / lightWidth : null);
        }
      }
      return lightSources;
    };
  ...
  /** @private */
  DefaultFurnitureCatalog.prototype.parseBoolean = function (aString) {
    return "true" == aString;
  }
   ...
   */
   DefaultFurnitureCatalog.PLUGIN_FURNITURE_CATALOG_FAMILY = "PluginFurnitureCatalog";
   DefaultFurnitureCatalog.CONTRIBUTED_FURNITURE_CATALOG_FAMILY = "ContributedFurnitureCatalog";
   DefaultFurnitureCatalog.ADDITIONAL_FURNITURE_CATALOG_FAMILY = "AdditionalFurnitureCatalog";
   DefaultFurnitureCatalog.furnitureAdditionalKeys = {};
   DefaultFurnitureCatalog.pluginFurnitureCatalogUrlUpdates = {};
