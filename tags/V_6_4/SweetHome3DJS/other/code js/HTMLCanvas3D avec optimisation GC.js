...
function HTMLCanvas3D(canvasId) {
  ...
  // Preallocated objects used in HTMLCanvas3D.prototype.drawGeometry to spare GC
...
HTMLCanvas3D.VEC3_ZERO = vec3.fromValues(0, 0, 0);
HTMLCanvas3D.VEC3_ONE = vec3.fromValues(1, 1, 1);
...
HTMLCanvas3D.prototype.prepareScene = function(node, displayedGeometries, background, parentLinks, lights, parentTransformations, 
  ...
  if (node instanceof Group3D) {
    ...
    if (onprogression !== undefined
      ...
      onprogression(ModelLoader.BINDING_MODEL, "", displayedGeometries.length / displayedGeometryCount);
    }
...
HTMLCanvas3D.prototype.drawGeometry = function(displayedGeometry, viewPlatformInvertedTransform, ambientLightColor, 
  ...
  if (displayedGeometry.visible
    ...
    this.gl.uniformMatrix4fv(this.shaderProgram.modelViewMatrix, false, this.geometryModelViewMatrix);
    ...
    if (lightingEnabled) {
      ...
      this.gl.uniform3fv(this.shaderProgram.ambientColor, this.geometryAmbientColor);

      var geometrySpecularColor = HTMLCanvas3D.VEC3_ZERO;
      if (!this.ignoreShininess
        ...
        geometrySpecularColor = displayedGeometry.specularColor;
      }
      this.gl.uniform3fv(this.shaderProgram.vertexSpecularColor, geometrySpecularColor);

      mat3.fromMat4(this.geometryNormalMatrix, this.geometryModelViewMatrix);
      this.gl.uniformMatrix3fv(this.shaderProgram.normalMatrix, false, this.geometryNormalMatrix);
      this.gl.uniform1i(this.shaderProgram.backFaceNormalFlip, displayedGeometry.backFaceNormalFlip);
    } 
    
    var geometryDiffuseColor = HTMLCanvas3D.VEC3_ONE;
    if (textureEnabled 
        && displayedGeometry.texture !== undefined) {
      ...
      if (displayedGeometry.diffuseColor !== undefined) {
        geometryDiffuseColor = displayedGeometry.diffuseColor;
      }
    }
    this.gl.uniform3fv(this.shaderProgram.vertexDiffuseColor, geometryDiffuseColor);
