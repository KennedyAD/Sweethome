...
const BINDING_MODEL = "Binding";
const READING_MODEL = "Reading";
const PARSING_MODEL = "Parsing";
const BUILDING_MODEL = "Building";
 ...
 *            modelError(err), modelLog(part, info, percentage) methods that
 *            will called at various phases.
 */
OBJLoader.prototype.load = function(uri, modelObserver) {
  ...
  var loader = this;
  modelObserver.modelLog(READING_MODEL, uri, NaN);
      ...
      if (objEntryName == null) {
        ...
        for (var i in entries) {
          if (entries [i].name.match(/\.obj$/)) {
            modelObserver.modelLoaded(loader.parseOBJEntry(entries [i], zip, modelObserver.modelLog));
            return;
          } 
        }
      } else {
        modelObserver.modelLoaded(loader.parseOBJEntry(zip.file(objEntryName), zip, modelObserver.modelLog));
      }
    } catch (err) {
      modelObserver.modelError(err);
    }
 ...
 */
OBJLoader.prototype.createScene = function(vertices, textureCoordinates, normals, groups, appearances, logCallback) {
  var sceneRoot = new Group3D();
  var groupCount = Object.keys(groups).length;
  var builtGroupCount = 0;
  for (var key in groups) {
    // Log each time 10% more groups are built
    if (Math.floor((builtGroupCount - 1) / groupCount * 10) < Math.floor(builtGroupCount / groupCount * 10)) {
      logCallback(BUILDING_MODEL, "", builtGroupCount / groupCount);
    }
            ...
            if (group.smooth) {
              ...
              for (var j = 0, index = 0; j < geometryCount; index += stripCounts [j], j++) {
                ...
                for (var k = 0; k < stripCount; k++) {
                  var vertexIndex = coordinatesIndices [index + k];
                  var vertex = vertices [vertexIndex];
                  vec3.sub(vector1, vertices [coordinatesIndices [index + (k < stripCount - 1 ? k + 1 : 0)]], vertex);
                  vec3.sub(vector2, vertices [coordinatesIndices [index + (k > 0 ? k - 1 : stripCount - 1)]], vertex);
                  ...
                  var sharedVertex = {"faceIndex" : j, "normal" : normal};
              ...
              for (var j = 0, index = 0; j < geometryCount; index += stripCounts [j], j++) {
                ...
                for (var k = 0; k < stripCount; k++) {
                  var vertexIndex = coordinatesIndices [index + k];
                  var defaultNormal = normals [normalIndices [index + k]];
                  ...
                  normals [normalIndices [index + k]] = normal;
                }
              }
            } else {
              for (var j = 0, index = 0; j < geometryCount; index += stripCounts [j], j++) {
                vec3.sub(vector1, vertices [coordinatesIndices [index + 1]], vertices [coordinatesIndices [index]]);
                vec3.sub(vector2, vertices [coordinatesIndices [index + stripCounts [j] - 1]], vertices [coordinatesIndices [index]]);
                var normal = vec3.cross(vec3.create(), vector1, vector2);
                vec3.normalize(normal, normal);
                // Add normal to normals set
              ...
              if (stripCounts [j] == 3) {
                ...
                while (polygonVertexIndices.length > 3) {
                  ...
                  for (var k = 0; k < polygonVertexIndices.length; k++) {
                    ...
                    vec3.sub(vector1, vertices [nextVertexIndex], vertex);
                    var ear = false;
                    if (vector1 [0] === 0 && vector1 [1] === 0 && vector1 [2] === 0) {
                      ...
                      vec3.sub(vector2, vertices [previousVertexIndex], vertex);
                      if (vector2 [0] === 0 && vector2 [1] === 0 && vector2 [2] === 0) {
                        ear = true;
                      } else if (vec3.dot(vector1, vector2) >= 0) {
...
/**
 * Parses and returns the scene described in the given OBJ entry.
 */
OBJLoader.prototype.parseOBJEntry = function(objEntry, zip, logCallback) {
  var vertices = [];
  ...
  var objText = objEntry.asBinary();
  var startOfLine = 0;
  var nextLoggedPercentage = 0.;
  while (startOfLine <= objText.length) {
    // Log each time 10% more lines are parsed
    if (nextLoggedPercentage <= startOfLine / objText.length) {
      logCallback(PARSING_MODEL, objEntry.name, startOfLine / objText.length);
      nextLoggedPercentage += 0.1;
    }
    var endOfLine = startOfLine + 1;
    while (endOfLine < objText.length
           && objText.charAt(endOfLine) != '\n'
           && objText.charAt(endOfLine) != '\r') {
      endOfLine++;
    }
    var line = objText.substring(startOfLine, endOfLine);
    if (line.indexOf("v") === 0 || line.indexOf("f ") === 0 || line.indexOf("l ") === 0) {
      ...
      while (line.charAt(line.length - 1) === '\\') {
        ...
        if (startOfLine < objText.length
            && objText.charAt(endOfLine) == '\r'
            && objText.charAt(startOfLine) == '\n') {
          startOfLine++;
        }
        ...
        line += objText.substring(startOfLine, endOfLine);
      }
    }
  ...
  if (nextLoggedPercentage < 1) {
    logCallback(PARSING_MODEL, objEntry.name, 1);
  }

  return this.createScene(vertices, textureCoordinates, normals, groups, appearances, logCallback);  
}
  ...
  for (var i in lines) {
    ...
    if (start === "newmtl") {
      ...
      if (start === "Ka") {
        ...
        if (imageEntry !== null) {
          var base64Image = btoa(imageEntry.asBinary());
          var image = new Image();
          ...
          currentAppearance.textureImage = image;
        }
      } 
...
ModelManager.loadModel = function(content, modelObserver) {
  ...
  if (modelRoot != null) {
    ...
    if (observers !== undefined) {
      ...
      new OBJLoader().load(content, 
          {
            modelLoaded: function(model) {
              var observers = ModelManager.loadingModelObservers [content];
              delete ModelManager.loadingModelObservers [content];
              ModelManager.loadedModelNodes [content] = model;
              for (var i in observers) {
                observers [i].modelUpdated(model);
              }
            },
            ...
            modelLog: function(part, info, percentage) {
              for (var i in ModelManager.loadingModelObservers [content]) {
                observers [i].modelLog(part, info, percentage);
              } 
            },
          });
    }
 ...
 */
Canvas3D.prototype.setScene = function(scene, logCallback) {
  var shapes = [];
  var shapeCount = this.countShapes(scene);
  this.prepareScene(scene, shapes, mat4.create(), logCallback, shapeCount);
  logCallback(BINDING_MODEL, "", 1);
  this.shapes = shapes;
  this.drawScene();
}

Canvas3D.prototype.countShapes = function(node) {
  if (node instanceof Group3D) {
    var shapeCount = 0;
    for (var i in node.children) {
      shapeCount += this.countShapes(node.children [i]);
    }
    return shapeCount;
  } else if (node instanceof Shape3D) {
    return 1;
...
Canvas3D.prototype.prepareScene = function(node, shapes, parentTransformations, logCallback, shapeCount) {
  if (node instanceof Group3D) {
    if (node instanceof TransformGroup3D) {
      parentTransformations = mat4.clone(parentTransformations);
      mat4.mul(parentTransformations, parentTransformations, node.transform);
    ...
    // Compute the bounds of all the node children
    for (var i in node.children) {
      this.prepareScene(node.children [i], shapes, parentTransformations, logCallback, shapeCount);
    }
  } else if (node instanceof Shape3D) {
    // Log each time 10% more shapes are bound
    if (Math.floor((shapes.length - 1) / shapeCount * 10) < Math.floor(shapes.length / shapeCount * 10)) {
      logCallback(BINDING_MODEL, "", shapes.length / shapeCount);
    }
    ...
    for (var i = 0; i < node.geometries.length; i++) {
      ...
      shape.vertexBuffer = this.prepareBuffer(geometry.vertices, geometry.vertexIndices);
      shape.textureCoordinatesBuffer = this.prepareBuffer(geometry.textureCoordinates, geometry.textureCoordinateIndices);
      if (geometry instanceof IndexedTriangleArray3D) {
        shape.mode = this.gl.TRIANGLES;
        shape.normalsBuffer = this.prepareBuffer(geometry.normals, geometry.normalIndices);
        shapes.push(shape);
      } else {
...
Canvas3D.prototype.bindTexture = function(texture) {
  ...
  if (Math.pow(texture.image.width, 0.5) !== Math.floor(Math.pow(texture.image.width, 0.5))
      && Math.pow(texture.image.height, 0.5) !== Math.floor(Math.pow(texture.image.height, 0.5))) {
...
Canvas3D.prototype.prepareBuffer = function(data, indices) {
  if (indices.length > 0) {
    ...
    for (var i in indices) {
      dataArray.set(data [indices [i]], i * itemSize);
    }
...
Canvas3D.prototype.drawScene = function() {
  ...
  for (var i in this.shapes) {
    ...
    var shapeModelViewMatrix = mat4.clone(this.viewPlatformTransform);
    ...
    this.gl.drawArrays(shape.mode, 0, shape.vertexCount);
  }
}

Canvas3D.prototype.setViewPlatformTransform = function(viewPlatformTransform) {
...
/**
 * Creates a model preview canvas.
 * @constructor
 */
function ModelPreviewComponent(id, pitchAndScaleChangeSupported) {
  Canvas3D.call(this, id);
  this.defaultViewPlatformTransform = this.viewPlatformTransform;
  ...
  if (pitchAndScaleChangeSupported) {
    const ANGLE_FACTOR = 0.02;
    const ZOOM_FACTOR = 0.02;
    var lastMouseMove = {"buttonPressed" : -1, "mouseInComponent" : false};
    var canvas = document.getElementById(id);
    var canvas3D = this;
    // TODO check on Safari iOS/IE
    canvas.addEventListener("mousedown", function(ev) {
        lastMouseMove.x = ev.clientX;
        lastMouseMove.y = ev.clientY;
        lastMouseMove.buttonPressed = ev.button;
      });
    canvas.addEventListener("mousemove", function(ev) {
        if (lastMouseMove.buttonPressed === 0) {
          if (ev.altKey) {
            canvas3D.viewScale = Math.max(0.5, Math.min(1.3, canvas3D.viewScale * Math.exp((ev.clientY - lastMouseMove.y) * ZOOM_FACTOR)));
          } else {
            canvas3D.viewYaw = canvas3D.viewYaw + ANGLE_FACTOR * (ev.clientX - lastMouseMove.x);
            canvas3D.viewPitch = Math.min(Math.PI / 4, Math.max(0, canvas3D.viewPitch + ANGLE_FACTOR * (ev.clientY - lastMouseMove.y)));
          }
          canvas3D.updateViewPlatformTransform();
        }
        lastMouseMove.x = ev.clientX;
        lastMouseMove.y = ev.clientY;
      });
    canvas.addEventListener("mouseup", function(ev) {
        lastMouseMove.x = ev.clientX;
        lastMouseMove.y = ev.clientY;
        lastMouseMove.buttonPressed = -1;
      });
    canvas.addEventListener("mouseleave", function(ev) {
        // TODO manage mouse leave + mouse up out of canvas
      });
    canvas.addEventListener("mouseenter", function(ev) {
      });
    canvas.addEventListener("DOMMouseScroll", function(ev) {
        canvas3D.viewScale = Math.max(0.5, Math.min(1.3, canvas3D.viewScale * Math.exp(-ev.detail * ZOOM_FACTOR)));
        canvas3D.updateViewPlatformTransform();
      });
  }
}
ModelPreviewComponent.prototype = Object.create(Canvas3D.prototype);
ModelPreviewComponent.prototype.constructor = ModelPreviewComponent;

ModelPreviewComponent.prototype.updateViewPlatformTransform = function() {
  var viewPlatformTransform = mat4.rotateX(mat4.create(), this.defaultViewPlatformTransform, this.viewPitch);
  mat4.rotateY(viewPlatformTransform, viewPlatformTransform, this.viewYaw);
  mat4.scale(viewPlatformTransform, viewPlatformTransform, vec3.fromValues(this.viewScale, this.viewScale, this.viewScale));
  this.setViewPlatformTransform(viewPlatformTransform);
}

/**
 * Loads and displays the given model.
 */
ModelPreviewComponent.prototype.setModel = function(content, errorCallback, logCallback) {
  var canvas = this;
  ModelManager.loadModel(content,
      {
        modelUpdated: function(model) {
          ...
          canvas.setScene(scene, logCallback);
        },
        modelError: function(err) {
        ...
        modelLog: function(part, info, percentage) {
          if (logCallback !== undefined) {
            logCallback(part, info, percentage);
          }
        }
      });
}